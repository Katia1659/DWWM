...
//Initialisation des filiere
for ($i =1 ; $i <= $numFil ; $i++){
    $filiere = "filiere$i";
    $code = $i;
    $libelle = readline("Nom de la filiere ? :");
...
for ($i = 1 ; $i <= $numEtud ; $i++){
    ...
    $$etudiant = new Etudiant($nom, $prenom, $dateNaissance, $$filiere);
}
...
for ($i = 1; $i <= $numFil; $i++) {
    ...
    for ($j = 1; $j <= $numEtud; $j++) {
        $etudiant = "etudiant$j";
        $libEtudiant = $$etudiant->_filiere->getLibelle();
