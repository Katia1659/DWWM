...
function arrayToCsv(string $fichier )  : array {

    $fichier = file_get_contents($fichier);
    $newTableau = json_decode($fichier, true);

    $keys = array_keys($newTableau);
    $values = array_values($newTableau);
...
// function replaceBy(string $file) {
